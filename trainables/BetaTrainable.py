...
class Trainable:
    ...
    def __init__(self, config=None, procedures=None):
    ...
    def train(self, x, y, features):
        ...
        with alive_bar(title="Executing Training Procedure", bar='classic') as bar:
            scorers = tt.compile_scorers(self.config, sklearn_compatible=True)
            self.train_results = self.validator(self.model, scorers, x, y)
    ...
    def fit(self, x, y, features):
        ...
        with alive_bar(title="Executing Training Procedure", bar='classic') as bar:
            scorers = tt.compile_scorers(self.config, sklearn_compatible=False)
            self.model.fit(x, y)

            y_preds = pyt.expand_0D_np_array(self.model.y_preds)
            y_true = pyt.expand_0D_np_array(self.model.y_true)

            # Score the models performance
            scores = {name: scorer(y_true, y_preds) for name, scorer in scorers.items()}

            # organise into eswults collection for downstream processing e.g diagnostics
            self.fit_results = dict()
            self.fit_results['y_preds'] = [pyt.expand_0D_np_array(self.model.y_preds)]
            self.fit_results['y_true'] = [pyt.expand_0D_np_array(self.model.y_true)]
